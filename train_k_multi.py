...
#        self.triplanes = nn.Parameter(torch.rand(1, 48*3, 128, 128))
        ...
        self.ddpm_pipeline = KPipeline('configs/config_64_cars_noisy.json', device='cuda:0')
...
def train(rank, world_size, transfer=""):
    ...
    if transfer == "":
        ...
        ckpt = torch.load(os.path.join(transfer, 'k_model.pth'))#, map_location=map_location)
    ...
    for e in range(start_epoch, num_epochs):
            ...
            if step % n_sample == 0:
                ...
                for vv in [1,2]:
                    ...
                    for k in range(len(img1)):
                        ...
                        imwrite(f'output/full-{step:06d}-{vv}-{k}.png', output)
                        if use_wandb:
                    ...
                    for i in range(len(samples)):
                        ...
                        imwrite(f'output/sample-{step:06d}-{vv}-{i}.png', s)
                        if use_wandb:
        ...
        if e % epochs_plot_loss == 0:
            #pass
            torch.save({'optim':optimizer.state_dict(), 'model':model.state_dict(), 'step':step, 'epoch':e}, "output/k-multi-latest.pt")
            #torch.save({'optim':optimizer.state_dict(), 'model':model.state_dict(), 'step':step, 'epoch':e}, f"epoch-{e}.pt")
        if e % epochs_plot_loss2 == 0:
            torch.save({'optim':optimizer.state_dict(), 'model':model.state_dict(), 'step':step, 'epoch':e}, f"output/k-multi-epoch-{e}.pt")
